...
> **MACs must be written with zero-padded octets** (`00:1a:2b:...`, never `0:1a:2b:...`). The daemon parses a MAC-subtype LLDP identifier by falling back to string parsing, and the abbreviated form fails with `InvalidLength` — so an abbreviated chassis MAC makes the whole neighbour record unparseable and **no LLDP data persists at all**, silently. Every fixture used the abbreviated form until 2026-07-27, which meant the sim had never actually exercised MAC-based LLDP matching.
