...
> **The stored MAC is not reproducible here**, for exactly the reason the FDB and ARP MACs are not simulated: `pass` is line-based and emits `string`, so a `PhysAddress` arrives as ASCII rather than six raw octets and `value_to_mac` correctly rejects it. **Every simulated interface in this environment stores a NULL `mac_address`.** See for yourself:
>
> ```bash
> snmpwalk -v2c -c netdefault -Ox 192.168.7.244 1.3.6.1.2.1.2.2.1.6   # 17 octets of ASCII, not 6
> ```
>
> The lines are kept because they are what the device reports, but nothing downstream of collection is exercised by them. Do not read a healthy L2 view here as evidence that the shared-MAC guard works. That rule is covered by unit tests — `find_if_entry_by_mac` and `find_host_by_mac` in `server/snmp/resolution/`, and `plan_interface_ip_links` in `server/hosts/service/discovery.rs` — and by any real device that repeats one MAC across its ports, which is common enough that most networks have one (a multi-radio printer or AP will usually do).
...
**`1/0/4` is the only subtype-3 port id in the lab.** It advertises `lldpRemPortIdSubtype` = 3 (`macAddress`) with `00:AD:24:AF:4E:00` — the address switch-dlink-01 reports on all three of its ports. It proves subtype 3 parses and reaches a lookup, and that a port id which resolves nothing still leaves an amber `NeighborLink` rather than no edge at all. Its `lldpRemPortDesc` is `Uplink to core`, matching no `ifName` or `ifDescr` on that switch: the port-description tier still runs after a failed port id (a description that *does* match should win), so anything matchable there would resolve the port and hide the case.

What it does **not** prove is the ambiguity guard. Because `ifPhysAddress` cannot survive `pass`, switch-dlink-01 stores no interface MACs, so this resolves as `port_not_found`, not `port_ambiguous` — the same amber edge for a different reason. Check the counter on the `LLDP/CDP link resolution complete` line rather than the edge colour if that distinction is what you are testing.
