...
impl ServiceDefinition for Veeam {
    ...
    /// Veeam Backup Enterprise Manager is commercial, with no pullable image and no published
    /// source, so a match string for its web UI would be written from documentation rather than
    /// from a response we have seen.
