//! Port subscriber for the historical Discovery row's
//! `EntityOperation::Created` event. Pulls `ScannedEntityIds` off the
//! in-memory event scope and backfills `last_discovery_id` /
//! `first_discovery_id` on the port rows the daemon scanned.

use std::collections::HashMap;

use async_trait::async_trait;

use crate::server::ports::service::PortService;
...
#[async_trait]
impl Subscriber<EntityOperation> for PortService {
        ...
        for event in events {
            if let Some((scanned, discovery_id)) = extract_scanned_from_discovery_event(&event) {
                <Self as DiscoveryFkUpdater<
                    crate::server::ports::r#impl::base::Port,
...
inventory::submit!(SubscriberRegistration::new::<PortService, EntityOperation>());
