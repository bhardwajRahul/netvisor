//! Interface subscriber for the historical Discovery row's
//! `EntityOperation::Created` event. Pulls `ScannedEntityIds` off the
//! in-memory event scope and backfills `last_discovery_id` /
//! `first_discovery_id` on the interface rows the daemon scanned.
...
#[async_trait]
impl Subscriber<EntityOperation> for InterfaceService {
