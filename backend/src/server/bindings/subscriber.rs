...
#[async_trait]
impl Subscriber<EntityOperation> for BindingService {
        ...
        for event in events {
            if let Some((scanned, discovery_id)) = extract_scanned_from_discovery_event(&event) {
                <Self as DiscoveryFkUpdater<
                    crate::server::bindings::r#impl::base::Binding,
...
inventory::submit!(SubscriberRegistration::new::<BindingService, EntityOperation>());
